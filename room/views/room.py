...
def get_user_type(user):
    if user.is_student:
        ...
        return 'Представитель организатора'
    elif user.is_staff:
        return 'Админ'
...
def browse_courses(request):
    ...
    if 'search' in request.GET:
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                UserLog.objects.create(action=f'Поиск "{query}"',
...
def contact_us(request):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    messages.success(request, 'Успешно отправлены данные. '
                                              'Спасибо.')
    ...
    return render(request, 'classroom/contact.html', {'title': 'Справка', 'form': form})
...
def login_view(request):
    ...
    return render(request, 'authentication/login.html', {'form': form, 'title': 'Вход'})
...
def register_page(request):
    if request.user.is_authenticated:
        return redirect('home')
    return render(request, 'authentication/register.html', {'title': 'Регистрация'})